...
When we independently apply the 2SLS estimation to the demand and supply equations, we can estimate $(\alpha_0, \alpha_1, \alpha_2)$ at first. Then by using instruments for the supply equation, we obtain the predicted value of $Q_t$, denoted as $\hat{Q}_t$, in the first-stage, and then we can estimate the parameters in the supply equation by regressing
\begin{align*}
...
The endogenous variable is $Q_t$. The demand instrument is $(Z_t, Z_tQ_t, w_t, r_t, h_t, k_t)$ and the supply instrument is $(Z_t, Z_tQ_t, w_t, r_t, h_t, k_t)$.
