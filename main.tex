...
As for the linear specification, we follow the setting of \cite{perloff2012collinearity} \textcolor{blue}{The paper does not provide any details of construction of a demand shifter. We compare the model without and with a demand shifter.}
...
This result uses the same data generation process as \citet{perloff2012collinearity}. However, for the estimation, we complement what the authors do not mention on instrument construction.
...
\textcolor{blue}{Thus we can correctly replicate the result in \citet{perloff2012collinearity}.}
...
But no simulation result is close to the true values of the supply parameters and the conduct parameter.
...
The simulation setting is explained in Section \ref{sec:simulation_and_estimation_setting}.
...
%Table: Estimation result of log-linear model
\begin{table}[!htbp]
      \begin{center}
...
%Figure: histogram of the estimation result of log-linear model
\begin{figure}[!htbp]
  \begin{center}
...
% The Contour map of the GMM objective function
\begin{figure}[!htbp]
  \begin{center}
...
% The difference between the GMM under the true and the estimated
\begin{figure}[!htbp]
  \begin{center}
  \includegraphics[width = 0.45\textwidth]
...
For the values in $(0,1)$, the shape of the histograms is the same as Figure  \ref{fg:histogram_loglinear_loglinear_no_constraint}, but we can see the spike around $\theta = 0$ and $1$.
...
Thus, putting the parameter constraint on the conduct parameter makes the estimation imprecise especially when the true conduct parameter means markets are perfectly competitive or collusive.
For example, \cite{merel2009measuring} uses log-linear specifications, and the estimated $\theta$ is 0.002 with 0.007 standard error, which means perfect competitive markets. 
The estimation may fail in the boundary as in Figure \ref{fg:histogram_loglinear_loglinear_with_constraint}. 

In summary, the suggestion of log-linear specification in \cite{perloff2012collinearity} adds another problem in its estimation, and we conclude that only the linear model can achieve a proper estimation of the conduct parameter in homogeneous good markets. 


%Figure: histogram of the estimation result of log-linear model with constraint
\begin{figure}[!htbp]
  \begin{center}
  \subfloat[]{\includegraphics[width = 0.5\textwidth]
