...
%The crux of the matter is Section 4.2. Here is my hunch as to what is happening. This is a severe form of non-identification. Start with a true (theta,g). Then someone else proposes an alternative theta’. Then you can use the transformation F, which depends on theta’ and the true (theta,g), to find a new g’. If this intuition is correct, any value of theta’ is observationally equivalent to the true theta as you can pick g’ to make all the data the same. Being able to pick any theta^{*} is why I previously used the term "severe form of non-identification”. 

%3. 7, Unclear how definition of a singular transformation relates to F as F is not the first display equation of Section 4.1.1
...
% 概念的にには(\theta, g)の組み合わせがいっぱいあって，その中にはOEになるようなものもあるが，かと言って，separableであることだけをもって，常にOEになる組み合わせに絞られているとは限らない．
% bresnahanの論文中で考えている例だと識別できないようなパラメーターの組み合わせがある
% その一方で，separable demandだったとしてもmarginal costにうまくivが入れば，それで識別ができる例もある(appendixに載せてもよい)
% necessityは多分大丈夫．異なるgと\thetaの組み合わせがあってOEを導いているなら，separableな関数になっているという議論
%問題はsufficiencyで，serapableなdemandをgivenとしたときに，具体的にOEになるようなgとthetaの組み合わせを見つけられるか？という議論をする必要がある
%Lauの議論としては，まず，separableな関数を仮定する．その後に，もし，
...
%この式自体，g' = (\theta - \theta^{*}) \partial P/\partial Q + gと変換した場合，
% g'はgのtransformationになっていることを示していそう．つまり，g' = T(g)となるようなFが存在する． 
%やっぱこれは無理そう．あるものの変換であるとは，g(a) = (b) となるa,bに対して，g'(a) = g'(b)であることを要求するが，\partial P/\partial Q (Q, a) = \partial P/\partial Q (Q, b)が成立するとは限らない． 
