...
reuslt = simulation_2sls_simultaneous()
...
simulation_3sls_simultaneous()


#---------------------------------------------------------------------------------------------------------

## Replication of Hyde and Perloff (2012)
...
function GMM_estimation(T, P, Z, X_s, X_d, Ω)
    ...
    @expression(model, r_d[t = 1:T], P[t] - sum(α[k] * X_d[t,k] for k = 1:K_d) )
    @NLexpression(model, r_s[t = 1:T], P[t] - sum(γ[k] * X_s[t,k] for k = 1:K_s-1) + log(1 - θ*(α[2] + α[3] * X_s[t,end])))
...
function nonlinear_TwoSLS(parameter, data)
    ...
    for t = 1:T
        ...
        Z_ts = vcat(1, z[t], iv[t,:])
        Z_td = vcat(1, z[t], w[t], r[t])
    ...
    α_hat, γ_hat, θ_hat = GMM_estimation(T, P, Z, X_s, X_d, Ω)
