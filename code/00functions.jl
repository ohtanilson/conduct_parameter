...
function termination_status_code(status)
    ...
    status_code = [
        MathOptInterface.OPTIMAL,                   # 1
        MathOptInterface.INFEASIBLE,                # 2
        MathOptInterface.DUAL_INFEASIBLE,           # 3
        MathOptInterface.LOCALLY_SOLVED,            # 4
        MathOptInterface.LOCALLY_INFEASIBLE,        # 5
        MathOptInterface.INFEASIBLE_OR_UNBOUNDED,   # 6
        MathOptInterface.ALMOST_OPTIMAL,            # 7
        MathOptInterface.ALMOST_INFEASIBLE,         # 8
        MathOptInterface.ALMOST_DUAL_INFEASIBLE,    # 9
        MathOptInterface.ALMOST_LOCALLY_SOLVED,     # 10
        MathOptInterface.ITERATION_LIMIT,           # 11
        MathOptInterface.TIME_LIMIT,                # 12
        MathOptInterface.NODE_LIMIT,                # 13
        MathOptInterface.SOLUTION_LIMIT,            # 14
        MathOptInterface.MEMORY_LIMIT,              # 15
        MathOptInterface.OBJECTIVE_LIMIT,           # 16
        MathOptInterface.NORM_LIMIT,                # 17
        MathOptInterface.OTHER_LIMIT,               # 18
        MathOptInterface.SLOW_PROGRESS,             # 19
        MathOptInterface.NUMERICAL_ERROR,           # 20
        MathOptInterface.INVALID_MODEL,             # 21
        MathOptInterface.INVALID_OPTION,            # 22
        MathOptInterface.INTERRUPTED,               # 23
        MathOptInterface.OTHER_ERROR                # 24
    ]
    ...
    elseif estimation_method[1] == :mpec_separate
        ...
        if length(sample_violatiton_index) == T
            ...
            if LinearAlgebra.rank(Z_s' * Z_s) == L_s
                ...
                @variable(model, 0 <= MC[t = 1:T])
                for t = 1:T
                    @NLconstraint(model, exp(P[t]) == MC[t] + θ * (α_hat[2] + α_hat[3] * X[t, end])* exp(P[t]))
                end

                r = Any[];
                g = Any[];
                for t =1:T
                    push!(r, @NLexpression(model, log(MC[t]) - sum(γ[k] * X_s[t,k] for k = 1:K_s-1)))
                end
...
function generate_contour_set_of_GMM(parameter, data, theta_range, gamma_range)
    ...
    for θ_hat = theta_range, γ_hat = gamma_range
        ...
        error = P .- γ_hat .-sum(γ[k] .* X_s[:,k] for k = 2:K_s-1) + log.(1 .- θ_hat .*(α_1 .+ α_2 .* X_s[:, end]) ) 

        g = sum(Z_s[t,:] .* error[t] for t = 1:T)
