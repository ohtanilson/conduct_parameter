...
function GMM_estimation_separate(T, Q, P, Z, Z_s, Z_d, X, X_s, X_d, parameter, estimation_method::Tuple{Symbol, Symbol, Symbol})
    ...
    """
    Solve the GMM estimation with the tight tolerance and the true parameters
    """
    ...
    if length(sample_violatiton_index) == T
        ...
        if rank(Z_s' * Z_s) == L_s
            ...
            JuMP.set_optimizer_attribute(model, "tol", 10^(-14))
...
# Estimate the parameters with the true starting value
...
function GMM_estimation_separate(T, Q, P, Z, Z_s, Z_d, X, X_s, X_d, parameter, estimation_method::Tuple{Symbol, Symbol, Symbol})
    ...
    """
    Solve the GMM estimation with the tight tolerance and random starting values
    """
...
# Estimate the parameters with the true starting value and the loose tolerance
function GMM_estimation_separate(T, Q, P, Z, Z_s, Z_d, X, X_s, X_d, parameter, estimation_method::Tuple{Symbol, Symbol, Symbol})

    """
    Solve the GMM estimation with loose tolerance and the true parameters
    """
    ...
    if length(sample_violatiton_index) == T
        ...
        if rank(Z_s' * Z_s) == L_s
            ...
            model = Model(Ipopt.Optimizer)
            JuMP.set_optimizer_attribute(model, "tol", 10^(-6))
            JuMP.set_optimizer_attribute(model, "max_iter", 1000)
            #JuMP.set_optimizer_attribute(model, "acceptable_tol", 1e-12)
            JuMP.set_silent(model)
            JuMP.@variable(model, γ[k = 1:K_s-1], start = start_γ[k])
...
# Estimate the parameters with random starting values and the loose tolerance


function GMM_estimation_separate(T, Q, P, Z, Z_s, Z_d, X, X_s, X_d, parameter, estimation_method::Tuple{Symbol, Symbol, Symbol})

    """
    Solve the GMM estimation with loose tolerance and random starting values
    """
    
    @unpack θ, γ_0, γ_1, γ_2, γ_3, start_θ, start_γ = parameter

    # Generate the random starting values
    start_γ = [γ_0, γ_1, γ_2, γ_3] .+ rand(Uniform(-20, 20), 4) 
