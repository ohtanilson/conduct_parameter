...
for estimation_method = [(:mpec_linear, :non_constraint, :theta_constraint)]
    for t = [50, 100, 200, 1000, 2000, 5000, 10000], sigma =  [0.001, 0.5, 1, 2], theta = [0.05, 0.1, 0.2, 0.33, 0.5, 1.0], alpha2 = [0.5, 1.0, 5.0, 20.0]
        ...
        parameter = market_parameters_log(T = t, σ = sigma, α_2 = alpha2, θ_0 = theta)
        ...
        filename_end   = ".csv"
        file_name = filename_begin*string(t)*"_theta_"*string(theta)*"_alpha2_"*string(alpha2)*"_sigma_"*string(sigma)*filename_estimation*"_optimal_iv_"*filename_end
