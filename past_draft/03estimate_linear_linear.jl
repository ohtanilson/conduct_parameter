...
using JuMP, Ipopt
using CSV, DataFrames, Plots, RData
...
for t = [50, 100, 200, 1000], sigma =  [0.001, 0.5, 1, 2]
    ...
    histogram_gamma_1 = histogram(estimation_result.γ_1, title = "γ_1, T = $t, σ = $sigma")

    display(histogram_gamma_1)

    histogram_θ = histogram(estimation_result.θ, title = "θ, T = $t, σ = $sigma")
    ...
    filename_begin = "../conduct_parameter/past_draft/parameter_hat_table_linear_linear_n_"
    filename_end   = ".csv"
...
for t = [50, 100, 200, 1000], sigma =  [0.001, 0.5, 1, 2]
    ...
    histogram_gamma_1 = histogram(estimation_result.γ_1, title = "γ_1, T = $t, σ = $sigma")
    histogram_θ = histogram(estimation_result.θ, title = "θ, T = $t, σ = $sigma")
    display(plot(histogram_θ, histogram_gamma_1))
end
