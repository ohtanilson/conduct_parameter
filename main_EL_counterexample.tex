...
\begin{assumption}\label{assumption:exclusive_shifters}
    ...
    \end{align}
    where the last two equalities hold because $X^{s}_j$ is not an argument of $P$ and $X^{d}_i$ is not an argument of $MC$, respectively.
\end{assumption}
...
\begin{definition}\label{definition:non_identification}
    The conduct parameter and the marginal cost function are said to be not identified if 
...
\begin{definition}\label{definition:identification}
    Identification are said to hold if for any two distinct pairs of conduct parameter and any marginal cost function, there exists a pair of demand shifter $X^{d}$ and $X^{s}$ where the equilibrium quantity $Q^e$ is different, that is, $h_q(X^{d}, X^{s}) \ne h_q^{*}(X^{d}, X^{s})$.
\end{definition}
...
\end{align}
We exclude $X^{d}_2$ from \eqref{eq:demand_counterexample}, but it is still non-separable from Lau's original definition of separability in \eqref{eq:separable_demand} based on \cite{goldmanNote1964}.
