...
Non-identification implies that given the demand shifter and cost shifter, both $\mathcal{E}$ and $\mathcal{E}^{*}$ lead to the same equilibrium quantity.
Lau shows that non-identification implies that there is a transformation between the marginal cost function $MC$ and $MC^{*}$.
Then, as the transformation also works as the transformation of the equilibrium conditions between $\mathcal{E}$ and $\mathcal{E}^{*}$, 
an equilibrium quantity satisfied in $\mathcal{E}$ is also satisfied in $\mathcal{E}^{*}$, which implies that observational equivalence holds.
However, we point out that this transformation does not work properly.
When a demand rotation instrument varies so as not to change the equilibrium in $\mathcal{E}$, to keep observational equivalence, the transformation requires that the marginal cost function $MC^{*}$ should depend on the demand shifter, which violates the exclusive demand shifters assumption.
...
Lau's claim and our result characterize the non-identification of the conduct parameter and the marginal cost function by assuming that the inverse demand function is already identified.
However, we show that the inverse demand function that leads to non-identification of the conduct parameter and the marginal cost function are not identified.
In other words, when the observed data is generated by the inverse demand function that leads to the non-identification of the conduct parameter and the marginal cost function, we cannot identify the inverse demand itself, and hence the all model components cannot be identified. 
...
\begin{assumption}\label{assumption:exclusive_shifters}
    ...
    \begin{align}
        \frac{\partial P}{\partial X^{d}_{i}}(Q, X^{d}) \ne 0, \text{ and } \frac{\partial MC}{\partial X^{s}_{j}}(Q, X^{s}) \ne 0
    \end{align}
\end{assumption}
...
\begin{assumption}\label{assumption:unique_equilibrium}
    ...
    \end{align}
\end{assumption}
Note that as the above equation consists of the derNivative of the inverse demand and the marginal cost, by Assumption \ref{assumption:twice_differentiable}, the derivative of the equilibrium condition with respect to $Q$ is finite.
...
\begin{definition}\label{definition:non_identification}
    The conduct parameter and the marginal cost function are said to be non-identified if there are two distinct pairs of conduct parameter and marginal cost function, denoted as $(\theta, MC)$ and $(\theta^{*}, MC^{*})$, and for any $X^{d}$ and $X^{s}$, there exist an equilibrium quantity $Q^e$ that satisfies both
    \begin{align}
...
\begin{corollary}\label{corollary:identification}
    The conduct parameter and the marginal cost is said to be identified if for any two distinct pairs of a conduct parameter and a marginal cost function, there exists a pair of demand shifter $X^{d}$ and $X^{s}$ where the equilibrium quantity $Q^e$ is different, that is, $h_q(X^{d}, X^{s}) \ne h_q^{*}(X^{d}, X^{s})$.
\end{corollary}
...
\begin{align}
    P(Q, X^{d}) = \alpha_0 - \alpha_1 Q + \alpha_2X^{d}_1 + \alpha_3QX^{d}_1 + \alpha_4X^{d}_2, \label{eq:demand_bresnahan}
\end{align}
...
Thus, in this case, the conduct parameter can be identified only when the inverse demand function takes the form \eqref{eq:identification_separable_demand_lau}.

However, while Lau's claim has been regarded as the extension of \citet{bresnahanOligopoly1982}, his claim itself does not provide a clear economic intuition why separability of the inverse demand function is the key for the identification.
For example, when the inverse demand function is given as $P(Q, r(X)) = Qr(X) + s(Q) + r(X)$.
When the demand shifter is a vector, it can be verified that the inverse demand function is a separable. 
But even though the demand shifter can work as a demand rotation instrument, the conduct parameter and the marginal cost function are not identified.

%\begin{align}
%    \frac{\partial}{\partial Q}\frac{\frac{\partial P}{\partial X^{d}_i}}{\frac{\partial P}{\partial X^{d}_j}} = \frac{r_i(X)(Qr_j(X) + r_j(X)) - r_j(X)(Qr_i(X) + r_i(X))}{(Qr_j(X) + r_j(X))^2} = 0
%\end{align}


In contrast, we are going to show the following theorem, which is the formal version of Result \ref{result:main} in the introduction.
\begin{theorem}\label{theorem:identification_characterization}
    ...
    \begin{align}
        P(Q, X^{d}) = Q^{\alpha}r(X^{d}) + s(Q) \label{eq:nonidentification_inverse_demand}
    \end{align}
    where $\alpha \ne -\frac{1}{\theta}$ is a constant, and $r(\cdot)$ and $s(\cdot)$ are twice continuously differentiable functions.
\end{theorem}
As in Lau's claim, non-identification implies a separable inverse demand function.
...
Observe that when $\alpha \ne 0$, the demand shifter changes only the slope of the inverse demand function, and when $\alpha = 0$, the demand shifter changes only the intercept of the inverse demand function.
...
This emphasizes the idea of \citet{bresnahanOligopoly1982} in more general settings.

Hereafter, we prove Theorem \ref{theorem:identification_characterization} in the following steps.
First, we derive the necessary condition for non-identification based on Definition \ref{definition:non_identification}.
By rewiring Definition \ref{definition:non_identification}, we can obtain the transformation between the derivatives of $MC$ and $MC^{*}$ that leads to observational equivalence given an inverse demand function (Lemmna \ref{lemma:mc_transformation}).
...
\begin{proposition}\label{proposition:nonidentification_inverse_demand}
    The non-identification of the conduct parameter and the marginal cost function implies \eqref{eq:identification_condition_separable} does not hold for all $i  = 1, \ldots, K_d$, and $C_i(Q, X^{d})$ and $D_i(Q, X^{d})$ are independent of $X^{d}$ for all $i$.    
    This implies that the inverse demand function is given as
    \begin{align}
        P(Q, X^{d}) = Q^{\alpha}r(X^{d}) + s(Q)
    \end{align}
    where $\alpha \ne -\frac{1}{\theta}$ is a constant, and $r(\cdot)$ and $s(\cdot)$ are twice continuously differentiable functions.
\end{proposition}
See Appendix \ref{appendix:proof} for the proof.
...
When an equilibrium quantity $Q^e$ satisfies the equilibrium condition under $(\theta, MC)$, by applying the transformation to the equilibrium condition under $(\theta, MC)$, we have that
\begin{align}
...
In fact, \eqref{eq:mc_transformation_quantity} and the inverse demand function \eqref{eq:nonidentification_inverse_demand} can construct the transformation for the equilibrium conditions.
Therefore, we can have the following sufficient condition.
...
\paragraph{The role of demand rotation instrument:}
Proposition \ref{proposition:nonidentification_inverse_demand} emphasizes the role of demand rotation instrument.
To get the intuition of how the instrument relates the proof, consider the economy with a linear inverse demand with a demand rotation instrument and a linear marginal cost.
\citet{bresnahanOligopoly1982} consider this environment.
...
Then, consider the change in the demand rotation instrument.
...
As an example, under the inverse demand function \eqref{eq:demand_counterexample}, $C_i$ and $D_i$ are given as
\begin{align}
    C_i(Q, X^{d}) = \frac{\alpha_2 + \theta\alpha_3Q}{\alpha_2 + \theta^{*}\alpha_3Q}\quad \text{ and }\quad  D_i(Q, X^{d}) =  (\theta^{*} - \theta)(-\alpha + \alpha_3X^{d}).
\end{align}
...
\begin{align}
    \frac{\partial MC^{*}}{\partial X^{s}_j}(Q, X^{s}) = \mu(Q, X^{s}) \frac{\partial MC}{\partial X^{s}_j}(Q, X^{s}),\quad \forall Q, X^{s}, \label{eq:nonidentification_marginal_cost_lau}
\end{align}
where $\mu(Q, X^{s})$ depends only on $Q$ and $X^{s}$.
...
It can be verified that Lau derives both equations \eqref{eq:nonidentification_demand} and \eqref{eq:nonidentification_marginal_cost} in his proof, but relies solely on \eqref{eq:nonidentification_marginal_cost}.

Next, Lau does not explicitly define $\mu(\cdot)$ in his formulation, but it can be inferred that his $\mu(\cdot)$ corresponds to our $\lambda(\cdot)$.
However, he neglects a point that the function $\mu(\cdot)$ implicitly depends on the demand shifter $X^{d}$, and this dependence is not addressed in his analysis.
As long as the variable $X^d$ cannot be eliminated from $\mu(\cdot)$, the transformation $T$ between $MC$ and $MC^*$ will necessarily depend on $X^d$, contradicting Assumption \ref{assumption:exclusive_shifters}.
...
\end{proposition}
See Appendix \ref{appendix:proof} for the proof.
It shows that for any $s(Q)$, another function $\tilde{s}(Q) = s(Q) + C Q^{-\frac{1}{\theta}}$, where $C$ is a nonzero constant, leads to the same equilibrium quantity for any $X^{d}$ and $X^{s}$, which implies that the inverse demand function itself is not identified.
Note that here $\alpha$ also include the case where $\alpha = -\frac{1}{\theta}$.
Therefore, while Claim \ref{theorem_lau} shows that the conduct parameter and the marginal cost function are identified as all demand shifters work as demand rotation instrument, the inverse demand function \eqref{eq:identification_separable_demand_lau} cannot be identified.
...
\begin{example}\label{example:demand_rotation_instrument_does_not_help_identification}
    ...
    \begin{align}
        P(Q, X^{d}) = Q^{\alpha}r(X^{d}) + s(Q) + t(X^{d}).
    \end{align}
...
\begin{align}
    P(Q, X^{d}) = Q^{\alpha}r(X^{d}) + s(Q). \label{eq:inverse_demand_separable_step2}
\end{align}
...
%\begin{align}
%    C_i(Q, X^{d}) = \frac{Q^{\alpha}h'(X^{d}) + \theta^{*}\alpha Q^{\alpha}h'(X^{d})}{Q^{\alpha}h'(X^{d}) + \theta \alpha Q^{\alpha}h'(X^{d})} = \frac{1 + \theta^{*}\alpha}{1 + \theta\alpha}.
%\end{align}
%and
%\begin{align}
%    D_i(Q, X^{d}) & = \frac{\theta^{*} - \theta}{Q^{\alpha} + \theta \alpha Q^{\alpha}} \left[ s'(Q)Q^{\alpha} + Qk''(Q) Q^{\alpha} - s'(Q)\alpha Q^{\alpha} \right]\\
...
\begin{align}
    \hat{C}_i(Q, X^{d}) = \frac{Q^{\alpha}h'(X^{d}) + \theta^{*}\alpha Q^{\alpha}h'(X^{d})}{Q^{\alpha}h'(X^{d}) + \theta \alpha Q^{\alpha}h'(X^{d})} = \frac{1 + \theta^{*}\alpha}{1 + \theta\alpha}.
\end{align}
and
\begin{align}
    \hat{D}_i(Q, X^{d}) & = \frac{\theta^{*} - \theta}{Q^{\alpha} + \theta \alpha Q^{\alpha}} \left[ s'(Q)Q^{\alpha} + Qk''(Q) Q^{\alpha} - s'(Q)\alpha Q^{\alpha} \right]\\
    &= \frac{\theta^{*} - \theta}{1 + \theta\alpha} \left[\frac{d}{dQ}Qs'(Q)  -\alpha s'(Q) \right].
\end{align}
...
\begin{align}
    & \frac{\theta^{*} - \theta}{1 + \theta\alpha} \left[Qs'(Q) - \alpha s(Q) \right] + \frac{1 + \theta^{*}\alpha}{1 + \theta\alpha} \left[(1+\theta\alpha) Q^{\alpha}r(X^{d}) + s(Q) + \theta Qs'(Q)\right]\\
    = & (1 + \theta^{*}\alpha)Q^{\alpha}r(X^{d}) + \frac{(\theta^{*} - \theta + (1 + \theta^{*}\alpha)\theta)Qs'(Q) + (1 + \theta^{*}\alpha - (\theta^{*} - \theta)\alpha) s(Q)}{1 + \theta\alpha}\\
    = & (1 + \theta^{*}\alpha)Q^{\alpha}r(X^{d}) + \frac{(1 + \theta\alpha) s(Q) + \theta^{*}(1 + \theta\alpha)Qs'(Q) }{1 + \theta\alpha}\\
    = & (1 + \theta^{*}\alpha)Q^{\alpha}r(X^{d}) +s(Q) + \theta^{*}Qs'(Q),
\end{align}
...
Fix, $\alpha$, $r(X^{d})$, and $\theta$.
For any $s(Q)$, the marginal revenue function under $\theta$ is given by
\begin{align}
    &Q^{\alpha}r(X^{d}) +s(Q) + \theta(\alpha Q^{\alpha-1}r(X^{d}) + s'(Q))Q\\
    = & Q^{\alpha}r(X^{d})(1+ \theta\alpha) + \theta s'(Q)Q + s(Q)
\end{align}
...
\begin{align}
    & Q^{\alpha}r(X^{d})(1 + \theta\alpha) + \tilde{s}(Q) + \tilde{s}'(Q)Q\\
    = & Q^{\alpha}r(X^{d})(1 + \theta\alpha) + s(Q) + C Q^{-\frac{1}{\theta}} + \theta\left(s'(Q) -\frac{1}{\theta} C Q^{-\frac{1}{\theta}-1}\right)Q\\
    =& Q^{\alpha}r(X^{d})(1 + \theta\alpha) + s(Q) + \theta s'(Q)Q
\end{align}
...
%    &= \alpha (Q^e)^{\alpha - 1} \frac{\partial h_q}{\partial X^{d}_i}(X^{d}, X^{s})r(X^{d}) + (Q^e)^{\alpha} \frac{\partial r}{\partial X^{d}_i}(X^{d}) + s'(Q) \frac{\partial h_q}{\partial X^{d}_i}(X^{d}, X^{s})\\
%    & = \frac{\partial h_q}{\partial X^{d}_i}(X^{d}, X^{s})\left(\alpha (Q^e)^{\alpha - 1}r(X^{d}) + s'(Q)\right) + (Q^e)^{\alpha} \frac{\partial r}{\partial X^{d}_i}(X^{d}), \label{eq:derivative_of_h_p_with_respect_to_X_d_i}
%\end{align}
%and
%\begin{align}
%    \frac{\partial h_p}{\partial X^{s}_j}(X^{d}, X^{s}) & = \frac{\partial P}{\partial X^{s}_j}(Q^e, X^{d})\\
%    &= \alpha(Q^e)^{\alpha-1} \frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})r(X^{d}) + s'(Q) \frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})\\
%    & = \frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})\left(\alpha (Q^e)^{\alpha - 1}r(X^{d}) + s'(Q^e)\right). \label{eq:derivative_of_h_p_with_respect_to_X_s_j}
%\end{align}
%From \eqref{eq:derivative_of_h_p_with_respect_to_X_s_j}, we have
%\begin{align}
%    \frac{\frac{\partial h_p}{\partial X^{s}_j}(X^{d}, X^{s})}{\frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})} = \alpha (Q^e)^{\alpha - 1}r(X^{d}) + s'(Q^e).
%\end{align}
%By substituting this into \eqref{eq:derivative_of_h_p_with_respect_to_X_d_i}, we have
%\begin{align}
%    \frac{\partial h_p}{\partial X^{d}_i}(X^{d}, X^{s}) & = \frac{\partial h_q}{\partial X^{d}_i}(X^{d}, X^{s})\left(\alpha Q^{\alpha - 1}r(X^{d}) + s'(Q)\right) + Q^{\alpha} \frac{\partial r}{\partial X^{d}_i}(X^{d})\\
%    & = \frac{\partial h_q}{\partial X^{d}_i}(X^{d}, X^{s})\frac{\frac{\partial h_p}{\partial X^{s}_j}(X^{d}, X^{s})}{\frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})} + Q^{\alpha} \frac{\partial r}{\partial X^{d}_i}(X^{d}).
%\end{align}

%By rearranging this equation as  
%\begin{align}
%    \underbrace{\frac{\partial h_p}{\partial X^{d}_i}(X^{d}, X^{s})  - \frac{\partial h_q}{\partial X^{d}_i}(X^{d}, X^{s})\frac{\frac{\partial h_p}{\partial X^{s}_j}(X^{d}, X^{s})}{\frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})}}_{\equiv \phi_{ij}(X^{d}, X^{s})} & = h_q(X^{d}, X^{s})^{\alpha}\frac{\partial r}{\partial X^{d}_i}(X^{d}).
%\end{align}
...
%Therefore, the left-hand side is a known function of $X^{d}$ and $X^{s}$, although $\alpha$ and $r(\cdot)$ are unknown.
%It is easy to see that the variation in $X^{s}$ can be used to identify $\alpha$ as $h_q$ is known.
%Then, given $\alpha$, we can consider the following equation such that
%\begin{align}
%    \log \left(\frac{\partial h_p}{\partial X^{d}_i}(X^{d}, X^{s})  - \frac{\partial h_q}{\partial X^{d}_i}(X^{d}, X^{s})\frac{\frac{\partial h_p}{\partial X^{s}_j}(X^{d}, X^{s})}{\frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})}\right) - \alpha \log h_q(X^{d}, X^{s}) & = \log \frac{\partial r}{\partial X^{d}_i}(X^{d}).
%\end{align}
...
%\begin{align}
%    \frac{\frac{\partial h_p}{\partial X^{s}_j}(X^{d}, X^{s})}{\frac{\partial h_q}{\partial X^{s}_j}(X^{d}, X^{s})} &= \alpha Q^{\alpha - 1}r(X^{d}) + s'(Q)\\
%    & = \alpha Q^{\alpha - 1}\tilde{r}(X^{d}) + s'(Q) + \alpha c Q^{\alpha - 1}\\
%    & = \alpha Q^{\alpha - 1}\tilde{r}(X^{d}) + \tilde{s}'(Q),
%\end{align}
