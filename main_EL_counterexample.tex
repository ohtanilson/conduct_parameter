...
\end{align}
The only difference between \eqref{eq:demand_counterexample} and \eqref{eq:demand_bresnahan} is if $X^{d}_2$ is included in the demand shifter, which changes only the intercept of the inverse demand function.
As this inverse demand function does not take the form \eqref{eq:identification_separable_demand_lau}, Theorem \ref{theorem_lau} implies that the conduct parameter is not identified because $X^{d}$ is a scalar.
In other words, in addition to the change in $X^{d}_1$, the identification is possible because of the change in $X^{d}_2$.
However, the role of demand rotation instrument is mostly contributed from the change in $X^{d}_1$, and hence the role of $X^{d}_2$ is almost negligible, which leads to a conflict with the result of \citet{lau1982identifying}.\footnote{For example, consider a linear marginal cost function, then we can find the path of $X^{d}_1$ that does not change the equilibrium quantity under the inverse demand \eqref{eq:demand_counterexample}.}
...
Here, we showed only that the demand rotation breaks observable equivalence when $MC^{*}$ is a linear function, hence there is a possibility that under non-linear $MC$ and $MC^{*}$, $MC^{*}$ can achieve observable equivalence without depending on the demand shifter.
However, hereafter, we show that this observation can be generalized, and we cannot find any marginal cost function that leads to observable equivalence under the inverse demand \eqref{eq:demand_counterexample}.
